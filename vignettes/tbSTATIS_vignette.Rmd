...
We leverage the EBM framework to classify different TB disease severity states at time of TB diagnosis. A benefit of the EBM is we only require cross-sectional data to fit the model which aligns with observing individuals only at time of TB diagnosis. The EBM allows us to integrate all clinical information to obtain a predicted disease stage that we use as a proxy for TB severity. We present the tbSTATIS package to implement our method. We provide this tutorial as a guide for conducting data analysis with tbSTATIS. 
...
## Example Data Set   
We provide a synthetic data set tbSTATISData in the tbSTATIS package. We will use this data set to walk through an example of analyzing data with tbSTATIS to classify TB disease severity.     
...
### True event sequence
Here is the ground truth sequence of events that was used to generate TBData. There are 4 disease stages. Events X6, X1, and X3 occur simultaneously at stage 1: 
