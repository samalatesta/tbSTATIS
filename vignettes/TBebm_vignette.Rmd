...
print(measure_info)
```
The fit_ebm function returns a list. One object in the list called ml_seq is a data frame with 4 columns (measure, event_number, event_name, est_seq). The stage column in this data frame is the maximum likelihood sequence. We present the maximum likelihood sequence for TBebmData. We ran the model with only 3 start points and 250 iterations but in practice the minimum number of start points should be 10 with at least 2000 iterations. From this output, the maximum likelihood sequence is ({X6, X1, X3}, {X2}, {X4},{X5}) which is the same as our ground truth sequence. 
```{r fit ebm}
model1 = fit_ebm(data=TBebmData[,c(1,3:8)], p_vec=c(.95,.95,.95,.95,.95,.95), info=measure_info, nstart=3,initial_iter= 500)
...
### Likelihood ascent
After estimating the model, we assess model convergence by plotting the likelihood ascent for each start point using the plot_likes funtion. The loglikelihoods for all iterations can be accessed from the saved model object. For example, we saved our model as model1 above and we can call model1$loglikes which is a dataframe that contains all loglikelihoods across all iterations and start points. We present the likelihood ascent plot for estimating the model with TBebmData. We see all start points converged within the first 50 iterations. When running the EBM with real data, it is likely the EBM will not converge until at least 1000 iterations. When fitting the model we recommend checking convergence by plotting the likelihood ascent and if convergence is very slow or does not occur then to increase the total number of iterations and/or start points and rerun the model. 
...
### Bootstrap resamples
After fitting the model, we recommend constructing a positional variance diagram to visualize uncertainty in the maximum likelihood sequence. We provide the function bootstrap_seq to refit the model on a chosen number of resamples with replacement from the observed data. We recommend using at least 100 resamples. This function requires the same inputs as fit_ebm with the additional argument z which is the total number of bootstrap resamples. The function returns a data frame where each row represents the sequence estimated for 1 resample from our observed data. We ran bootstrap_seq with TBebmData and our estimated sequence above. We present the first 6 rows of output. If our model fits the data well, we expect a majority of the bootstrap sequences to be the same as our maximum likelihood sequence. Like with fit_ebm, in practice a mininum of 1000 iterations and 10 start points should be used. 
```{r bootstrap sequences}

boot_seqs = bootstrap_seq(z=25,data=TBebmData[,c(1,3:8)], p_vec=c(.95,.95,.95,.95,.95,.95), info=measure_info, nstart=1,initial_iter= 5)

head(boot_seqs)
```

### Positional variance diagram
After obtaining bootstrap sequences, we plot them in a PVD to visualize the location and degree of uncertainty for our maximum likelihood sequence using the function pvd. The function requires a data frame of bootstrap sequences and the maximum likelihood sequence as input.  The function returns a heatmap where the x-axis is disease stage and the y axis is each event in order of the maximum likelihood sequence. Each square in the plot represents the proportion of bootstrap sequences where each event occurs at each disease stage. Darker colors indicated a higher proportion. The disease stage estimated for each event in the maximum likelihood sequence is outlined in black. We expect the highest proportion for each event to occur at the stage where the event occurs in the maximum likelihood sequence.    
   
We provide the PVD for the bootstrap samples for TBebmData below. A majority of our boostrap sequences were equivalent to the maximum likelihood sequence. We observe uncertainty for events X3 to X5 where a small proportion of sequences estimated these events to occur one stage later than the maximum likelihood sequence. It is important to note that the x axis for the pvd ranges from 1 to 6 to account for the fact that some bootstrap sequences may have a different number of stages than our maximum likelihood estimate. The maximum number of stages is always equal to the total number of events; for our example data set this is 6. 
...
stagedf <- get_stage(data=TBebmData[,c(1,3:8)], est_seq, c(.95,.95,.95,.95,.95,.95))
