...
We leverage the EBM framework to classify different TB disease severity states at time of TB diagnosis. A benefit of the EBM is we only require cross-sectional data to fit the model which aligns with observing individuals only at time of TB diagnosis. The EBM allows us to integrate all clinical information to obtain a predicted disease stage that we use as a proxy for TB severity. We present the TBebm package to implement our method. We provide this tutorial as a guide for conducting data analysis with TBebm. 
...
```{r fit ebm}
model1 = fit_ebm(data=TBebmData[,c(3:8)], p_vec=c(.9,.9,.9,.9,.9,.9), clinical_info=measure_info, nstart=3,initial_iter= 250)
...
boot_seqs = bootstrap_seq(z=25,data=TBebmData[,c(3:8)], p_vec=c(.9,.9,.9,.9,.9,.9), clinical_info=measure_info, nstart=1,initial_iter= 250)
...
stagedf <- get_stage(data=TBebmData[,c(3:8)], est_seq, c(.9,.9,.9,.9,.9,.9))
