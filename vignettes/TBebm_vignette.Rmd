...
After obtaining bootstrap sequences, we plot them in a PVD to visualize the location and degree of uncertainty for our maximum likelihood sequence using the function pvd. The function requires a data frame of bootstrap sequences and the maximum likelihood sequence as input.  The function returns a heatmap where the x-axis the is disease stage and the y axis is each event in order of the maximum likelihood sequence. Each square in the plot represents the proportion of bootstrap sequences where each event occurs at each disease stage. Darker colors indicated a higher proportion. We expect the highest proportion for each event to occur at the stage where the event occurs in the maximum likelihood sequence. We provide the PVD for the bootstrap samples for TBebmData below. A majority of our boostrap sequences were equivalent to the maximum likelihood sequence. We observe uncertainty for events X3 to X5 where a small proportion of sequences estimated these events to occur one stage later than the maximum likelihood sequence. It is important to note that the x axis for the pvd ranges from 1 to 6 to account for the fact that some bootstrap sequences may have a different number of stages than our maximum likelihood estimate. The maximum number of stages is always equal to the total number of events; for our example data set this is 6. 
