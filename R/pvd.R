...
ml <- dplyr::arrange(ml,clinical_measure, event_number)
...
pvd_plot=ggplot2::ggplot(pvd, ggplot2::aes(x = rowname, y = colname, fill = value)) +
  ggplot2::geom_tile() + ggplot2::xlab("Disease Stage") + ggplot2::ylab("Clinical Measure") + ggplot2::scale_fill_gradient(name="",low = "white", high = "#3A68AB", breaks=c(0,.25,.5,.75,1), labels=format(c(0,.25,.5,.75,1))) + ggplot2::theme_bw() + ggplot2::theme(text=ggplot2::element_text(size=14, color="black"))
